...
- (instancetype)initWithMapView:(GMSMapView *)mapView
    ...
    if ((self = [super init])) {
        ...
        _kGMUMinClusterSize = 4;
        _kGMUMaxClusterZoom = 20;
        _kGMUAnimationDuration = 0.5;
        _markerIcon = nil;
    }
    return self;
}
...
- (BOOL)shouldRenderAsCluster:(id<GMUCluster>)cluster atZoom:(float)zoom {
    return cluster.count >= _kGMUMinClusterSize && zoom <= _kGMUMaxClusterZoom;
}
...
- (void)clearMarkersAnimated:(NSArray<GMSMarker *> *)markers {
    ...
    for (GMSMarker *marker in markers) {
        ...
        [CATransaction begin];
        [CATransaction setAnimationDuration:_kGMUAnimationDuration];
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, _kGMUAnimationDuration * NSEC_PER_SEC),
