//
//  CustomNonHierarchicalDistanceBasedAlgorithm.m
//  safetyApp
//
...
/* Copied from "GMUNonHierarchicalDistanceBasedAlgorithm.m"*/

#if !defined(__has_feature) || !__has_feature(objc_arc)
#error "This file requires ARC support."
#endif

#import "CustomNonHierarchicalDistanceBasedAlgorithm.h"
...
@interface CustomClusterItemQuadItem : NSObject<GQTPointQuadTreeItem>
...
@implementation CustomClusterItemQuadItem {
...
- (BOOL)isEqual:(id)object {
    ...
    CustomClusterItemQuadItem *other = (CustomClusterItemQuadItem *)object;
    return [_clusterItem isEqual:other->_clusterItem];
}

@end

#pragma mark CustomNonHierarchicalDistanceBasedAlgorithm

@implementation CustomNonHierarchicalDistanceBasedAlgorithm {
...
- (instancetype)init {
    if ((self = [super init])) {
        ...
        _kGMUClusterDistancePoints = 100;
        _kGMUMapPointWidth = 2.0;
    }
    return self;
}
...
- (void)addItems:(NSArray<id<GMUClusterItem>> *)items {
    ...
    for (id<GMUClusterItem> item in items) {
        CustomClusterItemQuadItem *quadItem = [[CustomClusterItemQuadItem alloc] initWithClusterItem:item];
...
- (NSArray<id<GMUCluster>> *)clustersAtZoom:(float)zoom {
    ...
    for (id<GMUClusterItem> item in _items) {
        ...
        // around it.
        double radius = _kGMUClusterDistancePoints * _kGMUMapPointWidth / pow(2.0, zoom + 8.0);
        ...
        for (CustomClusterItemQuadItem *quadItem in nearbyItems) {
