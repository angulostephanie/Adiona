    ...
    func request(options: [String: Any], callback: ((err: NSError?, response: NSHTTPURLResponse?, body: AnyObject?)->())?) {
        ...
        if( options["auth"] != nil && (options["auth"] as! [String: String]).count > 0) {
            ...
            if( auth["username"] != nil && auth["password"] != nil ) {
                ...
                var authEncoded = "\(username!):\(password!)".dataUsingEncoding(NSASCIIStringEncoding, allowLossyConversion: true)!.base64EncodedStringWithOptions(NSDataBase64EncodingOptions());
                print(authEncoded)
        ...
        let task = session.dataTaskWithRequest(urlRequest, completionHandler: {(body, response, err) -> Void in
            ...
            if let body = body {
                do {
                    if(response!.MIMEType == "text/html") {
                        ...
                        var json: AnyObject! = try NSJSONSerialization.JSONObjectWithData(body as NSData, options: NSJSONReadingOptions.MutableContainers)
